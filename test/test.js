...
describe("MI PRIMER TOKEN TESTING", function () {
  ...
  var nftTokenName = "Mi Primer NFT";
  ...
  async function deployNftSC() {
    ...
    nftIpfsCID = "QmZ7j7GnMeRG81cTfRfFvrv5yafFxFkD5zyXCgMN2LiLCH";
    nftContract = await deploySC(nftContractName, [nftTokenName, nftTokenSymbol, nftIpfsCID]);
  ...
  describe("Public Sale Smart Contract", () => {
    ...
    beforeEach(async () => {
      ...
      await publicSaleContract.setMPRTKN(miPrimerTokenContract.address);
      await publicSaleContract.setGnosisWalletAddress(gnosis.address);
    });
    ...
    describe("Compra grupo 3 de NFT: 21 - 30", () => {
      it("Disminuye balance de MiPrimerToken luego de compra", async () => {
        var tokenPrice = await publicSaleContract.getPriceById(25);
    ...
    describe("Depositando Ether para Random NFT", () => {
      ...
      it("Método falla la vez 31", async () => {
        ...
        .to.be.rejectedWith("No quedan Tokens NFT disponibles");
    });
      ...
      it("Envío de Ether falla la vez 31", async () => {
        ...
        ).to.be.revertedWith("No quedan Tokens NFT disponibles");
      });
